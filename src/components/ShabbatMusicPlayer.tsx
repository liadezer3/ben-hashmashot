...
import { Music, ListMusic, Sparkles, Settings2 } from "lucide-react";
...
import CustomPlaylistManager from "./music/CustomPlaylistManager";
...
const ShabbatMusicPlayer = ({ candleLighting, havdalah, userId }: ShabbatMusicPlayerProps) => {
  ...
  const [customPlaylists, setCustomPlaylists] = useState<Record<string, { uri: string; name: string }>>({});

  const fetchCustomPlaylists = useCallback(async () => {
    if (!userId) return;
    const { data } = await supabase
      .from("custom_playlists")
      .select("phase_key, spotify_uri, playlist_name")
      .eq("user_id", userId);

    if (data) {
      const map: Record<string, { uri: string; name: string }> = {};
      data.forEach((p) => {
        map[p.phase_key] = { uri: p.spotify_uri, name: p.playlist_name };
      });
  ...
  const currentCustomPlaylist = customPlaylists[selectedPlaylist] || null;
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
                ...
                {Object.entries(SHABBAT_PLAYLISTS).map(([key, playlist]) => (
                  <SelectItem key={key} value={key}>
                    <span className="flex items-center gap-2">
                      ...
                      {customPlaylists[key] && <span className="text-xs text-secondary">⭐</span>}
                    </span>
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {currentCustomPlaylist && platform === 'spotify' && (
                <p className="text-xs text-secondary">⭐ פלייליסט אישי: {currentCustomPlaylist.name}</p>
              )}
            </div>
          </div>
          ...
          {platform === 'youtube' ? (
            ...
            <SpotifyPlayer
              playlists={currentPlaylist.spotify}
              customPlaylist={currentCustomPlaylist}
            />
          )}
        </CardContent>
      </Card>
      ...
      {showCustomManager && userId && (
        <CustomPlaylistManager
          userId={userId}
          onPlaylistsChange={fetchCustomPlaylists}
        />
      )}
    </div>
  );
